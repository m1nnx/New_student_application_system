...
<title>List of New Appliants</title>
...
<nav class="navbar navbar-expand-sm bg-primary navbar-dark">
  ...
  <ul class="navbar-nav">
    <li class="nav-item  active"><a class="nav-link" href="newApp.php">NEW APPLICATION</a></li>
  ...
  <ul class="navbar-nav ml-auto">
	...
	<h3 align="center" >LIST OF NEW APPLICANTS</h3>
	<?php include("db-config.php");
	...
	FROM pendaftaran PD
	LEFT JOIN pemohon P
	ON P.IC=PD.IC WHERE STATUS is null;";
	$result = mysqli_query($conn, $sql);
	$row = mysqli_num_rows($result);
	if($row>0)
	{?><center><div style="width:90%;font-size:14px;" class="font-weight-normal">
		...
		{   
			...
			<td><a href="uploads/<?php echo $row["RESULT"]; ?>" target="_blank">View</a>&nbsp  &nbsp
				<a href="uploads/<?php echo $row["RESULT"]; ?>" download>Download<a/></td>
			<td><?php echo $row["STATUS"];?></td>
			<td><a href="editNew.php?IC=<?php echo $row['IC'];?>"><i class="fa fa-edit fa-fw " style="color:blue" alt="Edit"></i></a>
			<a href="deleteNew.php?IC=<?php echo $row['IC'];?>" onclick="return myFunction()"><i class="fa fa-trash fa-fw " style="color:red" alt="Delete"></i></a></td></tr><?php 
		}
	} else{
		echo "<br><p align=center>Sorry, there is no new application.</p><br>";
	}
...
<script>function myFunction(){var r=confirm("Are you sure you want to delete this new applicant from the database?");if(r==false){return false;}}</script>
